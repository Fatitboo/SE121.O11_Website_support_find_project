...
export default function CustomCheckBox({label, listItem, filterValueChecked, error, type, rules, name, selectedItem}){
    ...
    return (
        <>
            ...
            {/* <RadioGroup value={selected} onChange={setSelected}>
                {
                    listItem.map((item, index) => {
                        return (<RadioGroup.Option key={index} value={item.value}>
                            {({ checked }) => (
                                <>
                                    <div className="flex flex-row items-center cursor-pointer mt-1">
                                        <div>
                                            <div className="relative h-7 flex items-center">
                                                <div className="absolute bg-[#FFF] border border-[#808082] w-[18px] h-[18px] rounded-[10px]" style={{borderColor: `${error ? "#a9252b" : "#808082"}`}} color="#FFF"></div>
                                                <div className={`${checked ? "": "hidden"} flex items-center justify-center absolute bg-[#1967d2] w-[18px] h-[18px] rounded-[10px]`} color="#FFF"><BsCheck color="#FFF"/></div>
                                            </div>
                                        </div>
                                        <span className="pl-7 text-base select-none text-[#696969]">{item.name}</span>
                                    </div>
