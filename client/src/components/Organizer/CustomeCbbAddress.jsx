...
export default function CustomeCbbAddress({label, listItem, filterValueSelected, error, name, type, onblur, rules, placeHolder, selectItem, styleOuter, labelItemSelected}) {
  const [selected, setSelected] = useState(selectItem ? selectItem : {code: -1, name: ''})
  ...
  return (
    <>
        ...
        <div onBlur={onblur} tabIndex={0}>
            <div name={name} type={type} rules={rules} value={selected?.code} onClick={toggleStateDropdown} onBlur={() => {setVisible('none'); setOutline(false);}} tabIndex={-1} className='relative cursor-default'>
