    ...
    <>
        ...
        <div onBlur={onblur} tabIndex={0}>
            <div name={name} type={type} rules={rules} value={selected.id} onClick={toggleStateDropdown} onBlur={() => {setVisible('none'); setOutline(false);}} tabIndex={-1} className='relative cursor-default'>
                <div className='relative w-full cursor-default rounded-md bg-[#f9fbfc] py-3.5 px-5 text-left text-gray-900 shadow-sm border border-gray-300 focus:bg-white sm:text-base sm:leading-6' style={{borderColor: `${error ? "#a9252b" : outline ? "#2D2D2D" : "rgb(209 213 219)"}`, backgroundColor: `${outline ? 'white': '#f9fbfc'}`}}>
