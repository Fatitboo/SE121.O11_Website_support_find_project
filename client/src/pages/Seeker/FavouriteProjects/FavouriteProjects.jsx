...
import { getAllFavouriteProjectsAction, resetSuccessAction } from "../../../redux/slices/projects/projectsSlices";
import ProjectItem from "../FindProjects/ProjectItem";


function FavouriteProjects() {
    const dispatch = useDispatch();
    const [FavouriteProjectsList, setFavouriteProjectsList] = useState([]);
    ...
    useEffect(() => {
        setPages([...FavouriteProjectsList.filter(item => ((item?.fullName).toLowerCase().includes(filterKeyWord.toLowerCase())
            ||(item?.project?.projectName).toLowerCase().includes(filterKeyWord.toLowerCase())
            || item?.project?.occupations?.filter(i => i.toLowerCase().includes(filterKeyWord.toLowerCase())).length > 0))
            .slice(currentPage * 10, (currentPage + 1) * 10)])
    }, [currentPage, FavouriteProjectsList, filterKeyWord])
    useEffect(() => {
        if (isSuccess) {
    ...
    return (
        <div className="px-10 pb-0">
            ...
            {/* Start title of page  */}
            <div className="mb-8">
                <div className="font-medium text-3xl text-gray-900 mb-2 leading-10">Favourite Projects!</div>
                    ...
                    <div className="relative rounded-lg mb-8 bg-white shadow max-w-full px-3 pt-1 shrink-0 w-full">
                        <div className="relative w-full">
                            ...
                            <div className="relative flex justify-between items-center flex-wrap bg-transparent px-8 py-5">
                                ...
                                <div className="flex ">
                                    <div className="mr-1">Favourite projects: </div> <span>  {pages.length}</span>
                                </div>
                            </div>
                            ...
                            <div className="px-6 relative">
                                <div className="overflow-y-hidden overflow-x-auto">
                                    <div className="bg-[#f5f7fc] color-white border-transparent flex justify-between border-0 w-full text-blue-700 px-10">
                                        <div className="py-6">Favourite projects</div>
                                    ...
                                    <div className="relative w-full overflow-y-hidden overflow-x-hidden rounded-md mb-8 bg-white border-0 px-10 pt-3 grid grid-cols-1 gap-4">
                                        {pages.length !== 0 ?
                                            ...
                                            }) : <div className="text-center mt-10 col-span-2 ">Not have any Favourite projects!</div>
                                        }
                                    </div>
                                    <div className="list-none mt-10 flex items-center justify-center mb-4">
                                        <PaginationButtons
                                            totalPages={FavouriteProjectsList?.length / 10}
